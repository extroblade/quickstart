...
const YANDEX_ID = env.ym;
type YM = (tagID: number, ...parameters: EventParameters) => void;
export const ym = (...parameters: EventParameters) => {
  // @ts-ignore
  const metrika = window.ym as YM | undefined;

  if (!metrika) {
    return;
  }

  metrika(YANDEX_ID, ...parameters);
};
